<?php
  ...
  $churchs_en = array(
    'Songun', 'ChuMidum', 'Sion', 'SaitamaRock', 'ChibaLove', 'TokyoRock', 'ChuShinhwa',
    'TruthLove','HeavenFortune','LordSail','Huimangpi','ChonSong','Truth','Glory','NorthGlory',
    'Aehwa','OsakaLove','TokushimaLove','HolySpirit','LordHeart','Hope','Peace',
    'ChuHyesong','Pudu','OitaPudu','MyoungHwa','KagoshimaHwa','ChuDaun','Seiyou'
  );

  for($i=1;$i<=count($churchs);$i++){
    try {
        $sql = "INSERT INTO unit(church_id,unit,church_name,url,men,women) VALUES(:church_id,:unit,:church_name,:url,0,0);";
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':church_id',$i);
        $stmt->bindValue(':unit',0);
        $stmt->bindParam(':church_name',$churchs[$i-1]);
        $stmt->bindParam(':url',$churchs_en[$i-1]);
        $stmt->execute();
    } catch (PDOException $e) {
        echo $e->getMessage();
        exit;
